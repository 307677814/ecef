...
AQUAENTRYFULL(void, Frame, EvalA)(LPVOID object, LPCSTR code, LPCSTR url, int line, LPVOID callback, LPVOID params,BOOL async) {
	if (object) {
		...
		pplx::task_completion_event<CefListValue*> *pResp = new pplx::task_completion_event<CefListValue*>;
		auto notify = pplx::create_task(*pResp);
		arg->SetSize(5);
		auto buf = conv.from_bytes(code);
		auto vUrl = conv.from_bytes(url);
		arg->SetInt(0, frame->GetIdentifier());
		arg->SetString(1, buf);
		arg->SetString(2, vUrl);
		arg->SetInt(3, line);
		arg->SetInt(4, (int)pResp);
		browser->SendProcessMessage(PID_RENDERER, message);
		if (async == FALSE) {
			CefListValue *retval = notify.get();
			delete pResp;
			if (retval != NULL) {
				...
				if (type == -1) {
					...
					try
					{
						v2 = conv.to_bytes(v1.c_str());
					}
					catch (const std::exception &ex)
					{
						int len = WideCharToMultiByte(CP_ACP, 0, v1.c_str(), v1.length(), NULL, NULL, NULL, NULL);
						char *buf = new char[len + 1];
						WideCharToMultiByte(CP_ACP, 0, v1.c_str(), v1.length(), buf, len, NULL, NULL);
						buf[len] = 0;
						v2 = buf;
						delete[]buf;
					}
				...
				((LPFN_V8_EVAL)callback)(0, NULL, false, NULL, NULL, params, NULL);
			}
		}
		else {
			notify.then([pResp,callback,params](CefListValue *retval) {
				delete pResp;
				std::wstring_convert<std::codecvt<wchar_t, char, mbstate_t>> conv(::new std::codecvt<wchar_t, char, mbstate_t>(".936"));
				if (retval != NULL) {
					int type = retval->GetInt(0);
